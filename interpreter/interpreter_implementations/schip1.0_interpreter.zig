...
pub const Schip10Interpreter = struct {
        ...
        switch (cur_byte.u) {
            0x0 => {
                switch (@as(u8, @bitCast(next_byte))) {
                    ...
                    0xFE => {
                        self.res = .low_res;
                        try self.base.changeResolution(allocator, 64, 32);
                        ret_work = .resolution_changed;
                    },
                    0xFF => {
                        self.res = .high_res;
                        try self.base.changeResolution(allocator, 128, 64);
                        ret_work = .resolution_changed;
                    },
                    else => {
                        try self.base.error_handler.handleInterpreterError(
            ...
            0xF => {
                switch (@as(u8, @bitCast(next_byte))) {
                    ...
                    // Yes apparently this is what has to be done according to https://chip-8.github.io/extensions/#super-chip-10
                    0x29 => {
                        // Note: This only works because the fonts start at 0x0 in the interpreters memory
                        if (self.base.registers[cur_byte.l] == 1) {
